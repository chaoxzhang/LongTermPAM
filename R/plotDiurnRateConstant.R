...
scaleRateConstant<-function(diurnal.para){
  ...
  maxmin.res<-
    ...
    dplyr::summarise(
      across(
        .col=c('PQ','NPQ','NPQr','par_PAM','temp_PAM'),
...
#' Diurnal rate constant parameters data visualization with field note and sunlight time marked in the figure
#'
#' This function will visualize MONI-PAM diurnal rate constant parameters data (PQ,NPQ,NPQr) and PAR and temperature in one figure with 10 days as a group plotting for each head and the field note will be marked in the figure if the note file is available. At the same time, this function will also show  the sunlight time (colorful vertical lines) in the plot, sunrise and sunsetStart are 'indianred', solarNoon is 'goldenrod', and dawn and dusk are 'purple'
#'
#' @usage plotDiurnRateConstant(diurnal.para,fieldnote,fieldnote.data,save.path)
