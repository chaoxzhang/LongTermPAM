...
scaleYield<-function(diurnal.para){
  ...
  data<-diurnal.para %>%
    dplyr::select(plot.group,head,tree_num,head_tree,
                  YII,Phi_fD,Phi_NPQ,Phi_NPQr,Phi_NPQs,
                  par_PAM,temp_PAM)
  ...
  maxmin.res<-
    ...
    dplyr::summarise(
      across(
        .col=c('YII','Phi_fD','Phi_NPQ','Phi_NPQr','Phi_NPQs',
               'par_PAM','temp_PAM'),
  ...
  maxmin.res<-
    maxmin.res %>%
    mutate(scale.temp.a=(1-0.1)/(max.temp_PAM-min.temp_PAM)) %>%
    mutate(scale.temp.b=0.1-scale.temp.a*min.temp_PAM) %>%
    mutate(scale.par.a=(1-0.1)/(max.par_PAM-min.par_PAM)) %>%
    mutate(scale.par.b=0.1-scale.par.a*min.par_PAM)
...
#' Diurnal quantum yield parameters data visualization with fieldnote and sunlight time marked in the figure
#'
...
#'
#' @usage plotYield(diurnal.para,fieldnote,fieldnote.data,save.path)
          ...
          if (isTRUE(sum(na.omit(plot.data$YII))>0&
            ...
            p1<-ggplot(plot.data,aes(x=datetime))+
              ...
              geom_vline(aes(xintercept=dawn),na.rm = T,
                         color='purple',linetype=2)+
              geom_line(aes(y=TtoYII(temp_PAM)),color='darkorchid',na.rm = T,alpha=0.5)+
