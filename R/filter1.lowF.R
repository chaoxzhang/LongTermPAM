...
#' @param PAM.data a combined organized MONI-PAM data which is generated from [readPAM] function.
...
filter1.lowF<-function(PAM.data,
  ...
  ##<<- add a column 'flag1.lowF.YII' to flag empty data of YII
  ##<<- firstly all the data of column 'flag1.lowF.YII' will be 1,
  ##<<- since the good/kept data will be flagged as 1
  PAM.data$flag1.lowF.YII<-1
  ...
  ##<<- 2. when F or Fm is NA, flag 0 in column 'flag1.lowF.YII', and replace
  ##       YII as NA
  PAM.data$flag1.lowF.YII[is.na(PAM.data$F_)|is.na(PAM.data$Fm_)]<-0
  PAM.data[which(PAM.data$flag1.lowF.YII==0),c('F_','Fm_','YII')]<-NA
  ...
  PAM.data$flag.all<-PAM.data$flag1.lowF.YII
  if (save.file==T){
